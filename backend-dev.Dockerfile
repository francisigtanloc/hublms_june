...
COPY --chown=$PLUGIN_BUILD_UID:$PLUGIN_BUILD_GID ./plugins/hublms/backend/requirements/dev.txt /tmp/plugin-dev-requirements.txt
RUN . /baserow/venv/bin/activate && pip3 install -r /tmp/plugin-dev-requirements.txt

COPY --chown=$PLUGIN_BUILD_UID:$PLUGIN_BUILD_GID ./plugins/hublms/ $BASEROW_PLUGIN_DIR/hublms/
RUN /baserow/plugins/install_plugin.sh --folder $BASEROW_PLUGIN_DIR/hublms --dev
